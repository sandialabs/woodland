...
struct Exact::Integrands : public acorn::CallerIntegrands {
  ...
  void eval (const int n, const Real* pts, RPtr integrands) const override {
    eval(n, pts, nullptr, integrands);
  }

  bool supports_mult_by_R3 () const { return true; }

  Real permitted_r_min (const Real r_max) const override {
    return e.o.permitted_r_min_factor*r_max;
  }

  void eval_mult_by_R3 (const int n, CRPtr pts, CRPtr J_times_pdirs,
                        RPtr integrands) const override {
    eval(n, pts, J_times_pdirs, integrands);
  }

  void eval_shape_J (const Real pt[2], Real J[6]) const override {
    Real z, z_xy[2];
    e.d->get_surface(pt[0], pt[1], z, z_xy);
    J[0] = 1; J[1] = 0;
    J[2] = 0; J[3] = 1;
    J[4] = z_xy[0]; J[5] = z_xy[1];
  }
...
private:
  ...
  void eval (const int n, CRPtr pts, CRPtr J_times_pdirs,
...
void Exact::calc_stress (const Real x, const Real y, Real sigma_lcs[6]) const {
  ...
  for (int iy = 0; iy < o.nyr; ++iy)
    for (int ix = 0; ix < o.nxr; ++ix) {
      ...
      if (hfp)
        acorn::integrals::calc_hfp(w, io, poly, igs, sigma);
      else if (tensor_quad)
